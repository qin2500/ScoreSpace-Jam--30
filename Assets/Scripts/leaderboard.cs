...
public class leaderboard : MonoBehaviour
{
    ...
    public void mainMenu()
    {
        ...
        SceneManager.UnloadSceneAsync(SceneNames.LEADERBOARD).completed += (asynchronous) => 
        {
            if (!GlobalEvents.PlayerStartedMoving.Invoked()) SceneManager.UnloadSceneAsync(SceneNames.MAINMENU).completed += (asyncOperation) =>
            {
                SceneManager.LoadSceneAsync(SceneNames.MAINMENU, LoadSceneMode.Additive).completed += (asyncOperation) =>
                {
            ...
            else
            {
                ...
                {
                    SceneManager.UnloadSceneAsync(SceneNames.LEVELCONTROLLER).completed += (asyncOperation) =>
                    {
                        SceneManager.LoadSceneAsync(SceneNames.MAINMENU, LoadSceneMode.Additive).completed += (asyncOperation) =>
                        {
                            GlobalEvents.PlayerPause.uninvoke();
